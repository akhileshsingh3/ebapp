...
var LandingPageNavLinkSelected="TableOfContent?Content="; 
...
function loadPage(page,ClassNSub){ 
	...
	if(XMLHttpRequestObject){
		XMLHttpRequestObject.open("GET", "resources/Files/Chapters7.txt"); 
			...
			if (XMLHttpRequestObject.readyState == 4 && 
				XMLHttpRequestObject.status == 200) { 			 
					...
					if(page.trim().localeCompare("tableOfContent")==0){
						...
						//Chapters= getThisFromFile(fileContent,"Class8-Mathematics");
						//createTabOfContent(Chapters,"Class8-Mathematics");
					}					
	...
	var ul= document.createElement('ul');
	  for(j;j< navLinks.length;j++){		  
		  ...
		  if(navLinks[j].trim().localeCompare("Home")===0){
			  anchor.href="LandingPage?Content=Class8-Home";
			  anchor.innerText= navLinks[j].trim();
			  link.id="li_"+navLinks[j].trim();	  		
		  }
		  else if(navLinks[j].trim().localeCompare("Support")==0){	
				anchor.href="Support?Content=Class8-Support";
				anchor.innerText= navLinks[j].trim();
				link.id="li_"+navLinks[j].trim(); 	
			}else{	
				//alert(j+"here")
				anchor.href="TableOfContent?Content=Class8-"+navLinks[j].trim();
				//anchor.href="TableOfContent";
				anchor.innerText= navLinks[j].trim();
				link.id="li_"+navLinks[j].trim();  		
			}
...
function createTabOfContent(Chapters,ClassNSub){
    ...
    for(j;j<= Chapters.length;j++){
      	 ...
      	 var notesFileLocation="resources/Files/Class8_Mathematics_Notes/Chap1.docx";
        ...
        row.insertCell(3).innerHTML ="<a href=tutorial?Tutorial="+ClassNSub+":"+Chapters[j-1].trim().replace(/\s/g,"%20")+">Watch";
     }
}
